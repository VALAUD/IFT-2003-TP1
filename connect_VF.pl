...
jouer_tour(Tableau,X):-
    ...
    afficher(Tableau), nl,
    format("
    C'est le tour de ~w (~w). Dans quelle colonne souhaitez-vous placer le jeton? (1 � 4)~n",[Nom, Jeton]),
    read(No_colonne),

    %Ajouter le jeton � l'emplacement choisi par le joueur
...
% doit gerer le cas ici o� la col est pleine et user change de choix_colonne
...
verifier_victoire(X, Tableau):-
    ...
    joueur(X, Nom, _),
    format("partie termin�!~n~w a gagn�!~n", [Nom]),!.
